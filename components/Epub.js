...
onRelocated=function(visibleLocation){


if(_this.props.onLocationChange){
_this.props.onLocationChange(visibleLocation);
}
};var bounds=_reactNative.Dimensions.get("window");_this.state={toc:[],show:false,width:bounds.width,height:bounds.height,orientation:"PORTRAIT"};return _this;}_createClass(Epub,[{key:"componentDidMount",value:function componentDidMount(){this.active=true;this._isMounted=true;_reactNative.AppState.addEventListener('change',this._handleAppStateChange.bind(this));if(this.props.src){this._loadBook(this.props.src);}}},{key:"componentWillUnmount",value:function componentWillUnmount(){this._isMounted=false;_reactNative.AppState.removeEventListener('change',this._handleAppStateChange);this.destroy();}},{key:"componentWillUpdate",value:function componentWillUpdate(nextProps){if(nextProps.src!==this.props.src){this.destroy();}}},{key:"componentDidUpdate",value:function componentDidUpdate(prevProps){if(prevProps.src!==this.props.src){this._loadBook(this.props.src);}else if(prevProps.orientation!==this.props.orientation){}}},{key:"_loadBook",value:function _loadBook(bookUrl){__DEV__&&console.log("loading book: ",bookUrl);this.book=(0,_epubjs2.default)({replacements:this.props.base64||"none"});return this._openBook(bookUrl);}},{key:"_openBook",value:function _openBook(bookUrl,useBase64){var _this2=this;if(!this.rendition){this.needsOpen=[bookUrl,useBase64];return;}this.book.open(bookUrl).catch(function(err){console.error(err);});this.book.ready.then(function(){_this2.props.onReady&&_this2.props.onReady(_this2.book);});this.book.loaded.navigation.then(function(nav){if(!_this2.active||!_this2._isMounted)return;_this2.setState({toc:nav.toc});_this2.props.onNavigationReady&&_this2.props.onNavigationReady(nav.toc);});if(this.props.generateLocations!=false){this.loadLocations().then(function(locations){_this2.rendition.setLocations(locations);_this2.rendition.reportLocation();_this2.props.onLocationsReady&&_this2.props.onLocationsReady(_this2.book.locations);});}}},{key:"loadLocations",value:function loadLocations(){var _this3=this;return this.book.ready.then(function(){return _this3.book.locations.generate(_this3.props.locationsCharBreak||600).then(function(locations){return locations;});});}},{key:"_handleAppStateChange",value:function _handleAppStateChange(
